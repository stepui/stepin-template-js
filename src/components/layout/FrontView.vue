<script setup>
  ...
  const navList = [
    ...
    },
    {
      title: 'Business',
      children: [
        { title: 'Business', list: ['Contact Us', 'Cooperation', 'Support'] },
      ],
    },
    {
      title: 'About Us',
  ...
  const message = ref();
  function onClose() {
    message.value.setAttribute(
      'style',
      `margin-top: ${0 - message.value.clientHeight}px`
    );
  ...
  <div class="front-view min-h-screen flex flex-col">
    <div class="front-main bg-[#003f8c] text-text flex-1">
      <div class="front-header flex items-baseline py-md px-xl">
        ...
        >
          <div
            :class="`front-nav-item mx-base ${nav.children ? 'with-list' : ''}`"
          ...
          >
            ...
            <a-popover :mouseEnterDelay="0.1" v-else placement="bottom">
              ...
              <template #content>
                <div class="flex">
                  <div
                    class="not-[:first-child]:ml-lg"
                    v-for="group in nav.children"
                  >
                    <h3>{{ group.title }}</h3>
                    <div
