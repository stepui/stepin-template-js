<script setup>
  ...
  const props = defineProps({
    current: [HTMLElement, Object, String],
    options: {
      required: true,
      type: Array,
    },
...
<template>
  <Teleport to="body">
    <div class="guider" :style="`display: ${show ? 'static' : 'none'}`">
      <div
        class="guider-left"
        :style="`border-left: ${location.left - 2}px solid rgba(0, 0, 0, 0.25)`"
      ></div>
      <div
        class="guider-top"
        :style="`left: ${location.left - 2}pxwidth: ${
          location.width + 4
        }pxborder-top: ${location.top - 2}px solid rgba(0, 0, 0, 0.25)`"
      ></div>
      <div
        class="guider-right"
        :style="`left: ${
          location.left + location.width + 2
        }pxbackground-color: rgba(0, 0, 0, 0.25)`"
      ></div>
      <div
        class="guider-bottom"
        :style="`left: ${location.left - 2}pxwidth: ${
          location.width + 4
        }pxtop: ${location.top + location.height + 2}px`"
      ></div>
      <div
        ref="doc"
        class="guider-doc flex flex-col justify-between rounded-md"
        :style="`left: ${docPosition.left}pxtop:${docPosition.top}px`"
      >
        <div
          class="arrow"
          :style="`left: ${arrowStyle.left}pxtop: ${arrowStyle.top}pxborder-${
