<script setup>
  ...
  const groupList = [outlined, filled, twoTone];
  ...
  const edit = useAuth('edit', function (record) {
  ...
  const remove = useAuth('delete', function (record) {
  ...
  <div class="authority">
    ...
    >
      ...
      >
        ...
        <a-form-item
          required
          :name="pageTypeConfig[pageType].props"
        ...
        <a-form-item required name="target" label="打开窗口">
          <a-radio-group
            :disabled="pageType === 'link' || pageType === 'iframe'"
        ...
        <a-form-item required name="renderMenu" label="渲染菜单">
          <a-switch
            v-model:checked="formData.renderMenu"
            checked-children="是"
